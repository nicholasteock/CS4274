...
//						+ " AND office.location = \"NUS\" ) ";	
				...
				String query = " SELECT person.name, person.preference, shop.name, shop.type "
						...
						+ " AND ( shop.location = \"AmI\""
						+ " AND shop.type = \"book\" ) )"
						+ " OR ( ( person.location = \"DC2\""
						+ " AND person.preference = \"pc\" )"
						+ " AND ( shop.location = \"DC2\""
						+ " AND shop.type = \"pc\" ) )"
						+ " OR ( ( person.location = \"NSS\""
						+ " AND person.preference = \"gift\" )"
						+ " AND ( shop.location = \"NSS\""
						+ " AND shop.type = \"gift\" ) )";
//				String query = "SELECT person.name FROM person WHERE person.location = \"AmI\"";
//				long queryBegin = System.currentTimeMillis();
				String result =  cds.queryContext(psgName,query,reference);
